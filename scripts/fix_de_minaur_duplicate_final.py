...
Properly handles the winner constraint
"""

import psycopg2

def fix_de_minaur_duplicate_final():
    """Merge A de Minaur into Alex de Minaur, properly handling constraints"""
    ...
    try:
        ...
        # First, let's check what matches involve A de Minaur
        cur.execute("""
        ...
        print(f"Found {len(matches)} matches involving A de Minaur")
        for match in matches:
            print(f"  Match {match[0]}: P1={match[1]}, P2={match[2]}, Winner={match[3]}")
        
        # Update matches where A de Minaur is player1
        cur.execute("""
            UPDATE matches 
            SET player1_id = %s 
            WHERE player1_id = %s
        ...
        # Update matches where A de Minaur is player2
        cur.execute("""
            UPDATE matches 
            SET player2_id = %s 
            WHERE player2_id = %s
        ...
        # Update matches where A de Minaur is the winner
        cur.execute("""
            UPDATE matches 
            SET winner_id = %s 
            WHERE winner_id = %s
